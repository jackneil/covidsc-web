define('./workbox-28703916.js', ['exports'], function(t) {
  'use strict';
  try {
  ...
  const a = t =>
    new URL(String(t), location.href).href.replace(new RegExp(`^${location.origin}`), '');
  class c {
    constructor() {
      this.t = new Map();
    }
    ...
    handleRequest({ request: t, event: e }) {
      ...
      const { params: n, route: i } = this.findMatchingRoute({ url: s, request: t, event: e });
      let r,
        a = i && i.handler;
  ...
  let o;
  const h = () => (o || ((o = new c()), o.addFetchListener(), o.addCacheListener()), o);
  const u = {
  ...
  class q {
    constructor(t) {
      (this.m = t), (this.o = new y('workbox-expiration', 1, { onupgradeneeded: t => this.q(t) }));
    }
    q(t) {
    ...
    async setTimestamp(t, e) {
      const s = { url: (t = m(t)), timestamp: e, cacheName: this.m, id: this.R(t) };
    ...
    async expireEntries(t, e) {
      const s = await this.o.transaction('cache-entries', 'readwrite', (s, n) => {
        ...
        n = [];
      for (const t of s) await this.o.delete('cache-entries', t.id), n.push(t.url);
      return n;
    }
    R(t) {
      return this.m + '|' + m(t);
  ...
  const v = (t, e) => t.filter(t => e in t),
    ...
    L = async ({ cacheName: t, request: e, event: s, matchOptions: n, plugins: i = [] }) => {
    ...
    x = async ({
      ...
      const h = await (async ({ request: t, response: e, event: s, plugins: n = [] }) => {
        let i = e,
          r = !1;
        for (const e of n)
          if ('cacheWillUpdate' in e) {
            ...
            if (((i = await n.call(e, { request: t, response: i, event: s })), !i)) break;
          }
      ...
      const u = await self.caches.open(t),
        ...
        f = l.length > 0 ? await L({ cacheName: t, matchOptions: c, request: o }) : null;
      try {
        await u.put(o, h);
      } catch (t) {
    ...
    b = async ({ request: t, fetchOptions: e, event: n, plugins: i = [] }) => {
      if (
        ('string' == typeof t && (t = new Request(t)), n instanceof FetchEvent && n.preloadResponse)
      ) {
      ...
      try {
        ...
        for (const t of i)
          'fetchDidSucceed' in t &&
            (s = await t.fetchDidSucceed.call(t, { event: n, request: c, response: s }));
        return s;
      } catch (t) {
        for (const e of r)
  ...
  class T {
    constructor(t) {
      (this.m = f(t)), (this.K = new Map()), (this.M = new Map()), (this.O = new Map());
    }
    addToCacheList(t) {
      const e = [];
      for (const n of t) {
    ...
    async install({ event: t, plugins: e } = {}) {
      ...
      for (const [t, e] of this.K) a.has(e) ? n.push(t) : s.push({ cacheKey: e, url: t });
      const c = s.map(({ cacheKey: s, url: n }) => {
        const i = this.O.get(s),
          r = this.M.get(n);
        return this.T({ cacheKey: s, cacheMode: r, event: t, integrity: i, plugins: e, url: n });
      });
      return await Promise.all(c), { updatedURLs: s.map(t => t.url), notUpdatedURLs: n };
    }
    async activate() {
    ...
    async T({ cacheKey: t, url: e, cacheMode: n, event: i, plugins: r, integrity: a }) {
      const c = new Request(e, { integrity: a, cache: n, credentials: 'same-origin' });
      let o,
    ...
    createHandlerBoundToURL(t, e = !0) {
      if (!this.getCacheKeyForURL(t)) throw new s('non-precached-url', { url: t });
  ...
  const C = (t, e) => {
    ...
    for (const n of (function*(
      t,
      { ignoreURLParametersMatching: e, directoryIndex: s, cleanURLs: n, urlManipulation: i } = {},
    ) {
  ...
  (t.CacheFirst = class {
    ...
    async k(t, e) {
      const s = await b({ request: t, event: e, fetchOptions: this.D, plugins: this.P }),
        n = s.clone(),
        i = x({ cacheName: this.m, request: t, response: n, event: e, plugins: this.P });
      if (e)
        try {
          e.waitUntil(i);
        } catch (t) {}
    ...
    (t.ExpirationPlugin = class {
      constructor(t = {}) {
        ...
        (this.cachedResponseWillBeUsed = async ({
          ...
          (this.F = new Map()),
          t.purgeOnQuotaError && ((e = () => this.deleteCacheAndMetadata()), p.add(e));
      }
      S(t) {
        if (t === w()) throw new s('expire-custom-caches-only');
        let e = this.F.get(t);
        return e || ((e = new R(t, this.I)), this.F.set(t, e)), e;
      }
      A(t) {
        if (!this.N) return !0;
        const e = this.W(t);
      ...
      async deleteCacheAndMetadata() {
        for (const [t, e] of this.F) await self.caches.delete(t), await e.delete();
        this.F = new Map();
      }
    }),
    (t.NetworkFirst = class {
      constructor(t = {}) {
        if (((this.m = w(t.cacheName)), t.plugins)) {
          ...
          this.P = e ? t.plugins : [E, ...t.plugins];
        } else this.P = [E];
      ...
      async handle({ event: t, request: e }) {
        ...
        let r;
        if (this.B) {
          const { id: s, promise: a } = this.H({ request: e, event: t, logs: n });
          (r = s), i.push(a);
        }
      ...
      H({ request: t, logs: e, event: s }) {
        ...
        return {
          promise: new Promise(e => {
            n = setTimeout(async () => {
              e(await this.$({ request: t, event: s }));
            }, 1e3 * this.B);
          }),
          id: n,
        };
      ...
      async j({ timeoutId: t, request: e, logs: s, event: n }) {
        let i, r;
        try {
          r = await b({ request: e, event: n, fetchOptions: this.D, plugins: this.P });
        } catch (t) {
          i = t;
        }
        if ((t && clearTimeout(t), i || !r)) r = await this.$({ request: e, event: n });
        else {
          const t = r.clone(),
            s = x({ cacheName: this.m, request: e, response: t, event: n, plugins: this.P });
          if (n)
            try {
              n.waitUntil(s);
            } catch (t) {}
    ...
    (t.precacheAndRoute = function(t, e) {
      !(function(t) {
        D().addToCacheList(t),
          t.length > 0 &&
            (self.addEventListener('install', F), self.addEventListener('activate', W));
      })(t),
        A(e);
    }),
...
//# sourceMappingURL=workbox-28703916.js.map
